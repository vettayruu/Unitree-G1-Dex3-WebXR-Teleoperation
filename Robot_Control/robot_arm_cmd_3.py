...
Kd = [
    8.0, 0.1, 0.1, 0.1, 0.01, 0.01,  # leg left
    8.0, 0.1, 0.1, 0.1, 0.01, 0.01,  # leg right
    10.0, 1, 1,  # waist
...
class Custom:
    def __init__(self):
        ...
        self.q_init_target[0] = np.deg2rad(12.0)
        self.q_init_target[6] = np.deg2rad(12.0)
    ...
    def _precompute_sigmoid_lut(self):
        ...
        for step in range(1, self.interp_steps + 1):
            # ratio = self.get_quintic_ratio(step, self.interp_steps)
    ...
    def get_quintic_ratio(self, step, total_steps):
        t = step / total_steps
        # 这一行公式能保证起止点的速度和加速度均为 0
    ...
    def ControlLogic(self):
        ...
        # Teleoperation
            ...
            # 3. Control signal
            self._update_target_smooth(left_arm_data, right_arm_data, waist_data)
    ...
    def _update_target_smooth(self, left_data, right_data, waist_data):
        # alpha 为平滑系数，1.0 代表不平滑，数值越小越平滑
        a = self.alpha

        # Waist 平滑
        for i in range(3):
            raw_target = waist_data[0 + i]
            # 一阶低通滤波公式: y = (1-a)*y_old + a*x_raw
            self.q_target[12 + i] = (1.0 - a) * self.q_target[12 + i] + a * raw_target

        # Left Arm and Right Arm 平滑
        for i in range(7):
            # 左臂
            raw_left = left_data[1 + i]
            self.q_target[15 + i] = (1.0 - a) * self.q_target[15 + i] + a * raw_left

            # 右臂
            raw_right = right_data[1 + i]
            self.q_target[22 + i] = (1.0 - a) * self.q_target[22 + i] + a * raw_right
