...
class WebCamSender:
    def __init__(self, signaling_urls: list[str], channel: str):
        self.configs = {
            "signaling_urls": signaling_urls,
            "video": True,
            "audio": False,
        ...
        self.sendonly = Sendonly(channel_id=channel, **self.configs)
        self._is_connected = False
    ...
    def connect(self):
        try:
            print(f"WebCam connecting to Sora: {self.channel}")
            self.sendonly.connect()
            self._is_connected = True
            return True
        except Exception as e:
            print(f"Connection failed: {e}")
    ...
    def send_frames(self, frame):
        try:
            if frame is not None:
                self.sendonly._video_source.on_captured(frame)
                return True
        except Exception as e:
            print(f"Send frame error: {e}")
        return False
