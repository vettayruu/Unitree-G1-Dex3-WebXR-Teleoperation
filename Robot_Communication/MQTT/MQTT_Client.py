...
# # MQTT_UCLAB_SERVER = "133.6.254.50"
...
class MQTT_Client():
    ...
    def on_connect(self, client, userdata, flags, reason_code, properties):
        if reason_code == 0:
            ...
            print(f"⏱️ Time sync request sent to the server (t0={t0})...")
    ...
    def on_message(self, client, userdata, msg):
        if msg.topic == self.MQTT_TIME_PONG_TOPIC:
            try:
                ...
                if t0 and server_time:
                    rtt = t1 - t0
                    network_delay = rtt // 2
                    calibrated_server_time = server_time + network_delay

                    self.time_offset = calibrated_server_time - t1
                    print(
                        f"⏱️ [TIME SYNC SUCCESS] One-way delay(RTT/2): {network_delay}ms, Time Offset: {self.time_offset}ms")
                ...
                print(f"❌ Failed to parse time synchronization packet: {e}")
            return
        ...
        if msg.topic == self.MQTT_RECV_TOPIC:
            try:
                ...
                if from_dev_id and from_dev_id != self.USER_UUID:
                    print(f"------------------ {self.get_synced_datetime()} -------------------")
                    ...
                    for t in topics_to_sub:
                        ...
                        if self.USER_UUID:
                            print(f"🔌 User {self.USER_UUID} requested UNREGISTER.")
